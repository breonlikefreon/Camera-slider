...
AccelStepper stepper1(1, 2, 3); // (Type:driver, STEP, DIR)
...
void loop() {
  ...
  // pan potentiometer
  sliderPoss = analogRead(JoyX);
  // If potentiometer is turned left, move slider left
  if (sliderPoss > 600) {
    sliderPoss = map(sliderPoss, 600, 1024, 0, 3000);
    stepper2.setSpeed(sliderPoss); // Increase speed as turning
  }
  // If potentiometer is turned right, move slider right
  else if (sliderPoss < 400 ) {
    sliderPoss = map(sliderPoss, 400, 0, 0, 3000);
    stepper2.setSpeed(-sliderPoss); // Increase speed as turning
  }
  // If potentiometer in middle, no movement
  else {
